...
export function buildCarFromForm(input: CarFormInput, existing?: Car | null): Car {
  ...
  const id = (input.id || existing?.id || \`admin-\${brandSlug}-\${slugify(model)}-\${year}\`).trim();
  ...
  return {
    ...
    specs: {
      ...
      transmission: input.transmission?.trim() || existing?.specs.transmission || "Auto",
      drive: input.drive?.trim() || existing?.specs.drive || "FWD",
      fuel: input.fuel?.trim() || existing?.specs.fuel || "Petrol",
      consumption: input.consumption?.trim() || existing?.specs.consumption || "?",
    },
    sync: photos.length
      ? {
          source: "autohome",
          ...
          exchangeBank: "VTB",
          exchangeRateAt: existing?.sync?.exchangeRateAt || new Date().toISOString(),
...
console.log("auth + middleware + car-form done");
